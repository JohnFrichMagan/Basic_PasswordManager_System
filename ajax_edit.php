...
$category = isset($_POST['category']) ? sanitizeInput($_POST['category']) : 'Work';
...
// Get icon based on the new site name
...
// Update the password entry with icon
try {
    $stmt = $pdo->prepare("UPDATE passwords SET name = ?, url = ?, username = ?, encrypted_password = ?, notes = ?, category = ?, icon = ?, updated_at = NOW() WHERE id = ? AND user_id = ?");
    $result = $stmt->execute([$name, $url, $username, $encrypted, $notes, $category, $icon, $id, $_SESSION['user_id']]);
    
    if ($result) {
        logActivity($pdo, $_SESSION['user_id'], 'edit_password', "Edited password ID: $id - Name: $name");
        echo json_encode(['success' => true, 'message' => 'Password updated successfully']);
    } else {
        echo json_encode(['success' => false, 'message' => 'No changes made or password not found']);
    }
